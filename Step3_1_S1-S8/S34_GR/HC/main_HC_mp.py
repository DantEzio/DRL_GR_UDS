...
def sample_action(foreaction,observation):
    action=[]
    k=0
    # CC R1
    if observation['CC-storage'] > 0.1:
        a = 1
    elif observation['CC-storage'] <= 0.01:
        a = 0
    else:
        if foreaction[k] == 1:
            a = 1
    ...
    k+=1
    # JK R1
    if observation['JK-storage'] > 0.1:
        a = 1
    elif observation['JK-storage'] <= 0.01:
        a = 0
    else:
        if foreaction[k] == 1:
            a = 1
...
def test(raine,rainw,i,testid):
    ...
    while not done:
        ...
        at = sample_action(foreaction,storage_states)
        foreaction = at
        ...
        t += 1
        if np.mod(t,int(30/env_params['advance_seconds']))==0:
            test_history['time'].append(t)
            test_history['state'].append(observation)
