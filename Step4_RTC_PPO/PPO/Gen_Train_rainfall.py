...
## East and west rainfall data for training
raindatae = np.load('./test_rainfall/east.npy').tolist()
raindataw = np.load('./test_rainfall/west.npy').tolist()

rains=[]
for rainid in range(len(raindatae)):
    rain=[]
    for i in range(len(raindatae[0])):
        tem=[raindatae[0][i][0],str((float(raindatae[0][i][1])+float(raindataw[0][i][1]))/2)]
        rain.append(tem)
    rains.append(rain)

np.save('./training_rainfall/rainew.npy',np.array(rains))